...
namespace Tortuga.Drydock.Models.SqlServer
{
    public class FixAddIdentityColumn : FixItOperation
    {
        ...
        {
            ShowFixIt = m_TableVM.IsHeap == true;
        }
        ...
        protected override FixItVM OnFixIt()
        {
            ...
            change.AppendLine($"ALTER TABLE {m_TableVM.Table.Name.ToQuotedString()} ADD [Id] INT NOT NULL IDENTITY");
            change.AppendLine($"ALTER TABLE {m_TableVM.Table.Name.ToQuotedString()} ADD PK_{m_TableVM.Table.Name.Name} PRIMARY KEY (Id)");
            ...
            rollBack.AppendLine($"ALTER TABLE {m_TableVM.Table.Name.ToQuotedString()} DROP COLUMN [Id]");
